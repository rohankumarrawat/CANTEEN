...
class CanteenApp(ctk.CTk):
        ...
        def _draw_bg(event=None):
            ...
            for x in range(-H + 62, W + H, 110):
                bg_canvas.create_polygon(
                    ...
                    fill="#192A1C", outline="")
            # Ashoka Chakra — centred, muted gold
            cx, cy = W // 2, H // 2
            R = min(W, H) // 3
            FGOLD = "#6B5A1E"
            bg_canvas.create_oval(cx-R,    cy-R,    cx+R,    cy+R,    outline=FGOLD, width=5)
            bg_canvas.create_oval(cx-R+20, cy-R+20, cx+R-20, cy+R-20, outline=FGOLD, width=2)
            bg_canvas.create_oval(cx-16,   cy-16,   cx+16,   cy+16,   fill=FGOLD, outline="")
            spoke_out, spoke_in = R - 16, 20
            for i in range(24):
                ang = math.radians(i * 15 - 90)
                bg_canvas.create_line(
                    cx + int(spoke_in  * math.cos(ang)),
                    cy + int(spoke_in  * math.sin(ang)),
                    cx + int(spoke_out * math.cos(ang)),
                    cy + int(spoke_out * math.sin(ang)),
                    fill=FGOLD, width=2)

        bg_canvas.bind("<Configure>", _draw_bg)
        self.after(30, _draw_bg)

        # Tricolor strip (above canvas)
    ...
    def _page_report(self):
        ...
        # ── At-a-glance KPI bar (always visible, no scroll needed) ────────────
        ...
        # Paper card with slight elevation feel
        ...
        lbl(lh_i, "DAILY OPERATIONS REPORT",
            size=17, weight="bold", color=WHITE).pack(anchor="w", pady=(2, 0))
        lbl(lh_i, "सेवा परमो धर्म   •   " +
        ...
        # ── Section 1: Meal Sales ─────────────────────────────────────────────
        sc1 = ctk.CTkFrame(rc, fg_color="transparent")
        sc1.pack(fill="x", padx=PAD, pady=(20, 0))
        band(sc1, "Meal Sales Summary", bg=ARMY_BG, tc=GOLD_LT, h=42)
        ...
        for idx, row in enumerate(s_rows):
            ...
            trow(sc1,
                 ...
                 SWTS, bg=bg2)
        # Grand total row
        tot2 = ctk.CTkFrame(sc1, fg_color=BG_SAF, corner_radius=0, height=38)
        tot2.pack(fill="x")
        ...
        sc2 = ctk.CTkFrame(rc, fg_color="transparent")
        sc2.pack(fill="x", padx=PAD, pady=(20, 0))
        band(sc2, "Financial Summary", bg=ARMY_BG, tc=GOLD_LT, h=42)
        fin_f = ctk.CTkFrame(sc2, fg_color="transparent")
        fin_f.pack(fill="x", pady=10)
        ...
        for i, (icon, desc, val, tc2, bg_c, bdr) in enumerate([
                ...
                ("📈", "Net Daily Profit", f"₹ {net_profit:,.0f}", SAFFRON,BG_SAF, T_SAF)]):
        ...
        sc3 = ctk.CTkFrame(rc, fg_color="transparent")
        sc3.pack(fill="x", padx=PAD, pady=(20, 0))
        band(sc3, "Payment Mode Breakdown", bg=ARMY_BG, tc=GOLD_LT, h=42)
        pm_f = ctk.CTkFrame(sc3, fg_color="transparent")
        pm_f.pack(fill="x", pady=10)
        pm_f.grid_rowconfigure(0, weight=1)
        for i, (mode, amt, pct, clr, bg_c, bdr) in enumerate([
                ("💵  Cash", f"₹ {cash_a:,.0f}",
                 f"{cash_a/total_rev*100:.0f}% of total" if total_rev else "0%",
                 GREEN, BG_GRN, T_GRN),
                ("📱  UPI",  f"₹ {upi_a:,.0f}",
                 f"{upi_a/total_rev*100:.0f}% of total"  if total_rev else "0%",
                 PURPLE, BG_PUR, T_PUR),
                ("💳  Card", f"₹ {card_a:,.0f}",
                 f"{card_a/total_rev*100:.0f}% of total" if total_rev else "0%",
                 BLUE, BG_BLU, T_BLU)]):
        ...
        sc4 = ctk.CTkFrame(rc, fg_color="transparent")
        sc4.pack(fill="x", padx=PAD, pady=(20, 0))
        band(sc4, "Official Sign-Off", bg=ARMY_BG, tc=GOLD_LT, h=42)
        so_f = ctk.CTkFrame(sc4, fg_color="transparent")
        so_f.pack(fill="x", pady=10)
        so_f.grid_rowconfigure(0, weight=1)
        ...
        for i, (role, name, rank) in enumerate([
            ...
            sb3 = card(so_f, fg_color="#F9FAFB")
            sb3.grid(row=0, column=i, padx=(0 if i == 0 else 10), sticky="nsew")
            so_f.grid_columnconfigure(i, weight=1)
            lbl(sb3, role, size=10, weight="bold", color=MID).pack(
                padx=16, pady=(14, 2), anchor="w")
            lbl(sb3, name, size=12, weight="bold", color=ARMY_BG).pack(
                padx=16, pady=(0, 0), anchor="w")
            lbl(sb3, rank, size=10, color=MID).pack(
                padx=16, pady=(2, 10), anchor="w")
            # Signature area
            ctk.CTkFrame(sb3, height=1, fg_color=BORDER).pack(fill="x", padx=16)
            lbl(sb3, "Signature:", size=10, color=MID).pack(
                padx=16, pady=(6, 0), anchor="w")
            ctk.CTkFrame(sb3, height=38, fg_color="transparent").pack()
            ctk.CTkFrame(sb3, height=1, fg_color=BORDER).pack(fill="x", padx=16)
            lbl(sb3, f"Date:  {date_str}", size=10, color=MID).pack(
                padx=16, pady=(6, 14), anchor="w")

        # ── Footer ────────────────────────────────────────────────────────────
        ctk.CTkFrame(rc, height=1, fg_color=BORDER).pack(
            fill="x", padx=PAD, pady=(20, 0))
        foot_f = ctk.CTkFrame(rc, fg_color="transparent")
        foot_f.pack(fill="x", padx=PAD, pady=(6, 0))
        lbl(foot_f, "CONFIDENTIAL — For Official Use Only",
            size=10, weight="bold", color=MID).pack(side="left")
        lbl(foot_f, "Generated: " + datetime.now().strftime("%d %b %Y, %I:%M %p"),
            size=10, color=MID).pack(side="right")
        ft2 = ctk.CTkFrame(rc, fg_color="transparent", height=6)
        ft2.pack(fill="x", pady=(10, 0))
